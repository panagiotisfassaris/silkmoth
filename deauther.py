...
def sniff_macs(interface, duration=10):
    ...
    return list(aps), list(clients)

def choose_target_and_ap(aps, clients):
    print("\n[Access Points]")
    ...
    try:
        ap_index = int(input("\nSelect AP by number: "))
        client_index = int(input("Select Client by number: "))
        return clients[client_index], aps[ap_index]
...
"""
    ...
    parser = argparse.ArgumentParser(description="Wi-Fi Deauthentication & Scanning Tool")
    ...
    parser.add_argument("--interactive", action="store_true", help="Interactively select target and AP from scan")
    ...
    if args.scan or args.interactive:
        aps, clients = sniff_macs(args.interface)
        ...
        if args.interactive:
            args.target_mac, args.router_mac = choose_target_and_ap(aps, clients)

    if not args.target_mac or not args.router_mac:
        print("[!] You must provide both target and router MAC addresses unless using --interactive or --scan mode.")
        sys.exit(1)
